...
//many contact fields can be made optional (via the omitfields parameter)
		...
		$xml .= 'ea="' . sumac_convertXMLSpecialChars($_POST['email'])
		...
		foreach ($_POST as $checkbox_name => $pv)
		{
			...
			if (isset($_POST[$checkbox_name])){
					$id = str_replace('cpip_', '', $checkbox_name);
					if ($first){
						$xml .= sumac_convertXMLSpecialChars($id);
						$first = false;
					}
					else
						$xml .= '|' . sumac_convertXMLSpecialChars($id);
...
include_once 'sumac_xml.php';
		...
		else
		{
			...
			if ($responseStatus == 'good')
			{
				...
				echo sumac_getFunctionCompletedHTML($_SESSION[SUMAC_STR]["UB2"],$responseMessage,false,false,true);
			}
...
include_once 'sumac_update_contact_details.php';
			...
			{
				echo sumac_getFunctionCompletedHTML($_SESSION[SUMAC_STR]["UB1"],$responseMessage,false,false,true);
			}
		}
	}
