...
//version552//
	...
	define("SUMAC_CODE_VERSION" , '5.5.2');
	define("SUMAC_CODE_DATE" , '2014-05-27');
...
//	1. New online package for allowing user to just register their contact details ('signup') (using 'login2')
...
//these INFO, WARNING, and ERROR constants contain messages/values unsuitable for user alteration @@@? might they be localisable ?
	...
	define("SUMAC_INFO_FOOTER_SUMAC_DONATION2_TEXT",'Powered by Sumac Fundraising Software');
	define("SUMAC_INFO_FOOTER_SUMAC_DONATION2_THANKS_TEXT",'Powered by Sumac Donation Software');
	define("SUMAC_INFO_FOOTER_SUMAC_CONNECT2_TEXT",'Powered by Sumac Non-profit CRM');
	define("SUMAC_INFO_FOOTER_SUMAC_DEPART2_TEXT",'Powered by Sumac Non-profit CRM');
	define("SUMAC_INFO_FOOTER_SUMAC_DONATION2_LINK",'/fundraising-software');
	define("SUMAC_INFO_FOOTER_SUMAC_DONATION2_THANKS_LINK",'/donation-software');
	define("SUMAC_INFO_FOOTER_SUMAC_CONNECT2_LINK",'/product/how-it-works/');
	define("SUMAC_INFO_FOOTER_SUMAC_DEPART2_LINK",'/product/how-it-works/');
...
//using the values in the user-accessible file
	...
	if (isset($_SESSION[SUMAC_STR]) === false)
	{
		...
		$_SESSION['sumac_override_text_js'] = 'var sumac_override_sids = ['; //that starts the new javascript array variable ...
		$sa = array();
...
//echo $sakey . '=' . $savalue . '<br />';
			if ((substr($sakey,0,2) == 'D2') || (substr($sakey,0,2) == 'F2') || (substr($sakey,0,2) == 'S2'))
				$_SESSION['sumac_override_text_js'] .= '{id:"'.$sakey.'",str:"'.$savalue.'"},';
			else if (substr($sakey,0,2) == 'G2') $_SESSION['sumac_override_text_js'] .= '{id:"'.$sakey.'",str:"'.$savalue.'"},';
			else
			{
				if ($savalue == '') continue;
				$sa[$sakey] = $savalue;
		...
		$_SESSION['sumac_override_text_js'] .= '];';  // ... and that closes the array off.
