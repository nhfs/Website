...
function sumac_singleform2($previousRequest,$responseMessage,$formhandler,$loadedAccountDocument,$loadedFormTemplateElement)
{
	...
	$html = sumac_singleform2_HTML($formTemplateElement,$organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage);
	echo $html;
	return true;
}

function sumac_singleform2_HTML($formTemplateElement,$organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage)
{
	return '<!DOCTYPE html>'.PHP_EOL
			.sumac_geth2_head('singleform2')
			.sumac_singleform2_body($formTemplateElement,$organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage);
}

function sumac_singleform2_body($formTemplateElement,$organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage)
{
	$retryform = ($previousRequest != '') ? 'all' : null;

	return '<body>'.PHP_EOL
			.sumac_geth2_user('top','singleform2')
			.sumac_singleform2_content($formTemplateElement,$organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage)
			.sumac_geth2_user('bottom','singleform2')
			.sumac_geth2_body_footer('singleform2',true,$retryform,'')
			.'</body>'.PHP_EOL;
}

function sumac_singleform2_content($formTemplateElement,$organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage)
{
	...
	$html .= sumac_singleform2_HTMLform($formTemplateElement,$organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage);
	$html .= '</div>'.PHP_EOL;	//mainpage

	$html .= sumac_geth2_sumac_software_link('singleform2',SUMAC_INFO_FOOTER_SUMAC_CONNECT2_LINK,SUMAC_INFO_FOOTER_SUMAC_CONNECT2_TEXT);
...
function sumac_singleform2_HTMLform($formTemplateElement,$organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage)
{
	$html = sumac_geth2_formtag('singleform2','all','form',$formhandler);
//	$html .= sumac_login2('singleform2','O',false,$organisationDocument,$accountDocument,$previousRequest,$responseMessage,'L1');
	...
	$html .= sumac_login2('singleform2','O',false,$organisationDocument,$accountDocument,$previousRequest,$responseMessage,null);
...
function sumac_singleform2_get_checkbox_list($formField,$fieldContent,$id,$title)
{
	...
	for ($i = 0; $i < count($fieldValueArray); $i++)
	{
		...
		$html .= sumac_geth2_input_with_label('singleform2','cb_'.$id.'_'.$i,'checkboxlist',$id.'_'.$i,$attrs,$fv,'after',false);
	}
