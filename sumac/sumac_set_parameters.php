...
//set parameters common to all packages
 	...
 	if (($resume === false) || isset($getKeys['fonttype']))
		$_SESSION[SUMAC_SESSION_FONTS] = sumac_get_parameter('Arial,Helvetica,sans-serif','fonttype',(isset($getKeys['fonttype']) ? $getKeys['fonttype'] : null));
	...
	if (($resume === false) || isset($getKeys['attmtblock']))
		$_SESSION[SUMAC_USERPAR_ATTMTBLOCK] = sumac_get_parameter(SUMAC_ATTACHMENT_BLOCK_SIZE,'attmtblock',(isset($getKeys['attmtblock']) ? $getKeys['attmtblock'] : null));
	if ($_SESSION[SUMAC_USERPAR_ATTMTBLOCK] > SUMAC_ATTACHMENT_MAX_BLOCKSIZE) $_SESSION[SUMAC_USERPAR_ATTMTBLOCK] = SUMAC_ATTACHMENT_MAX_BLOCKSIZE;
...
// set parameters for the COURSES package alone
	...
	if (($_SESSION[SUMAC_SESSION_DEFAULT_SORT_COLUMN] > 4) || ($_SESSION[SUMAC_SESSION_DEFAULT_SORT_COLUMN] < 1))
	...
	$_SESSION[SUMAC_SESSION_ACTIVE_SORT_DIRECTION] = $_SESSION[SUMAC_SESSION_DEFAULT_SORT_DIRECTION];
 	...
 	if (($resume === false) || isset($getKeys['singlereg']))
		$_SESSION[SUMAC_USERPAR_SINGLEREG] = (substr(strtolower(sumac_get_parameter('false','singlereg',(isset($getKeys['singlereg']) ? $getKeys['singlereg'] : null))),0,1) != 'f');
...
// set new global/general login parameters
 	if ($resume === false)	//MUST COME FROM FILE
		$_SESSION[SUMAC_USERPAR_NOUSERPW]['login2'] = (strtolower(substr(sumac_get_parameter_from_file('false','g2nouserpw'),0,1)) !== 'f');
 	if ($resume === false)	//MUST COME FROM FILE
		$_SESSION[SUMAC_USERPAR_OMITFIELDS]['login2'] = strtolower(sumac_get_parameter_from_file('now','g2omitfields'));
 	if ($resume === false)	//MUST COME FROM FILE
		$_SESSION[SUMAC_USERPAR_MUSTFIELDS]['login2'] = strtolower(sumac_get_parameter_from_file('fatszcp','g2mustfields'));
 	if (($resume === false) || isset($getKeys['g2statesdd']))
		$_SESSION[SUMAC_USERPAR_STATESDD]['login2'] = sumac_get_parameter('','g2statesdd',(isset($getKeys['g2statesdd']) ? $getKeys['g2statesdd'] : null));

// set parameters for new donation package alone
 	if (($resume === false) || isset($getKeys['d2fixedamt']))
		$_SESSION[SUMAC_USERPAR_D2FIXEDAMT] = '';
		$d2fixedamt = sumac_get_parameter('','d2fixedamt',(isset($getKeys['d2fixedamt']) ? $getKeys['d2fixedamt'] : null));
		if (is_numeric($d2fixedamt)) $_SESSION[SUMAC_USERPAR_D2FIXEDAMT] = $d2fixedamt * 100;	//if valid, convert to cents
 	if (($resume === false) || isset($getKeys['d2amounts']))
		$_SESSION[SUMAC_USERPAR_D2AMOUNTS] = sumac_get_parameter('','d2amounts',(isset($getKeys['d2amounts']) ? $getKeys['d2amounts'] : null));
 	if (($resume === false) || isset($getKeys['d2frequency']))
		$_SESSION[SUMAC_USERPAR_D2FREQUENCY] = strtoupper(sumac_get_parameter('C','d2frequency',(isset($getKeys['d2frequency']) ? $getKeys['d2frequency'] : null)));
	//only first character will be checked - 'M' = monthly, 'C' = combined, 'O' = once
 	if (($resume === false) || isset($getKeys['d2nologin']))
		$_SESSION[SUMAC_USERPAR_D2NOLOGIN] = (strtolower(substr(sumac_get_parameter('false','d2nologin',(isset($getKeys['d2nologin']) ? $getKeys['d2nologin'] : null)),0,1)) !== 'f');
 	if (($resume === false) || isset($getKeys['d2asnavlnk']))
		$_SESSION[SUMAC_USERPAR_D2ASNAVLNK] = (substr(strtolower(sumac_get_parameter('false','d2asnavlnk',(isset($getKeys['d2asnavlnk']) ? $getKeys['d2asnavlnk'] : null))),0,1) != 'f');
 	if (($resume === false) || isset($getKeys['d2fundreqd']))
		$_SESSION[SUMAC_USERPAR_D2FUNDREQD] = (substr(strtolower(sumac_get_parameter('false','d2fundreqd',(isset($getKeys['d2fundreqd']) ? $getKeys['d2fundreqd'] : null))),0,1) != 'f');
 	if (($resume === false) || isset($getKeys['d2fundkey']))
		$_SESSION[SUMAC_USERPAR_D2FUNDKEY] = sumac_get_parameter('','d2fundkey',(isset($getKeys['d2fundkey']) ? $getKeys['d2fundkey'] : null));

//set these new donation _SESSION globals only if there is a parameter setting
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['donation2']);
		$temp = sumac_get_parameter_from_file(null,'d2nouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['donation2'] = (strtolower(substr($temp,0,1)) !== 'f');
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['donation2']);
		$temp = sumac_get_parameter_from_file(null,'d2omitfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['donation2'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['donation2']);
		$temp = sumac_get_parameter_from_file(null,'d2mustfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['donation2'] = strtolower($temp);
	}
 	if (($resume === false) || isset($getKeys['d2statesdd']))
 	{
		unset($_SESSION[SUMAC_USERPAR_STATESDD]['donation2']);
		$temp = sumac_get_parameter(null,'d2statesdd',(isset($getKeys['d2statesdd']) ? $getKeys['d2statesdd'] : null));
		if ($temp != null) $_SESSION[SUMAC_USERPAR_STATESDD]['donation2'] = $temp;
	}

// set parameters for new singleform package alone
//set these new singleform _SESSION globals only if there is a parameter setting
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['singleform2']);
		$temp = sumac_get_parameter_from_file(null,'f2nouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['singleform2'] = (strtolower(substr($temp,0,1)) !== 'f');
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['singleform2']);
		$temp = sumac_get_parameter_from_file(null,'f2omitfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['singleform2'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['singleform2']);
		$temp = sumac_get_parameter_from_file(null,'f2mustfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['singleform2'] = strtolower($temp);
	}
 	if (($resume === false) || isset($getKeys['f2statesdd']))
 	{
		unset($_SESSION[SUMAC_USERPAR_STATESDD]['singleform2']);
		$temp = sumac_get_parameter(null,'f2statesdd',(isset($getKeys['f2statesdd']) ? $getKeys['f2statesdd'] : null));
		if ($temp != null) $_SESSION[SUMAC_USERPAR_STATESDD]['singleform2'] = $temp;
	}

// set parameters for new login for each of the old packages individually
// they are donation, membership, contact, courses, and ticketing
//set _SESSION globals only if there is a parameter setting
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['donation']);
		$temp = sumac_get_parameter_from_file(null,'d1nouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['donation'] = (strtolower(substr($temp,0,1)) !== 'f');
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['donation']);
		$temp = sumac_get_parameter_from_file(null,'d1omitfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['donation'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['donation']);
		$temp = sumac_get_parameter_from_file(null,'d1mustfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['donation'] = strtolower($temp);
	}
 	if (($resume === false) || isset($getKeys['d1statesdd']))
 	{
		unset($_SESSION[SUMAC_USERPAR_STATESDD]['donation']);
		$temp = sumac_get_parameter(null,'d1statesdd',(isset($getKeys['d1statesdd']) ? $getKeys['d1statesdd'] : null));
		if ($temp != null) $_SESSION[SUMAC_USERPAR_STATESDD]['donation'] = $temp;
	}

 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['membership']);
		$temp = sumac_get_parameter_from_file(null,'m2nouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['membership'] = (strtolower(substr($temp,0,1)) !== 'f');
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['membership']);
		$temp = sumac_get_parameter_from_file(null,'m2omitfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['membership'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['membership']);
		$temp = sumac_get_parameter_from_file(null,'m2mustfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['membership'] = strtolower($temp);
	}
 	if (($resume === false) || isset($getKeys['m2statesdd']))
 	{
		unset($_SESSION[SUMAC_USERPAR_STATESDD]['membership']);
		$temp = sumac_get_parameter(null,'m2statesdd',(isset($getKeys['m2statesdd']) ? $getKeys['m2statesdd'] : null));
		if ($temp != null) $_SESSION[SUMAC_USERPAR_STATESDD]['membership'] = $temp;
	}
	//and repeat settings for 'membership2' package, i.e. same parameters used for both packages
	if (isset($_SESSION[SUMAC_USERPAR_NOUSERPW]['membership'])) $_SESSION[SUMAC_USERPAR_NOUSERPW]['membership2'] = $_SESSION[SUMAC_USERPAR_NOUSERPW]['membership'];
	else unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['membership2']);
	if (isset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['membership'])) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['membership2'] = $_SESSION[SUMAC_USERPAR_OMITFIELDS]['membership'];
	else unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['membership2']);
	if (isset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['membership'])) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['membership2'] = $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['membership'];
	else unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['membership2']);
	if (isset($_SESSION[SUMAC_USERPAR_STATESDD]['membership'])) $_SESSION[SUMAC_USERPAR_STATESDD]['membership2'] = $_SESSION[SUMAC_USERPAR_STATESDD]['membership'];
	else unset($_SESSION[SUMAC_USERPAR_STATESDD]['membership2']);

 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['contact']);
		$temp = sumac_get_parameter_from_file(null,'u2nouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['contact'] = (strtolower(substr($temp,0,1)) !== 'f');
	}
	//contact update never uses the 'create acoount' panel so the other three parameters arent needed

 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['courses']);
		$temp = sumac_get_parameter_from_file(null,'c2nouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['courses'] = (strtolower(substr($temp,0,1)) !== 'f');
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['courses']);
		$temp = sumac_get_parameter_from_file(null,'c2omitfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['courses'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['courses']);
		$temp = sumac_get_parameter_from_file(null,'c2mustfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['courses'] = strtolower($temp);
	}
 	if (($resume === false) || isset($getKeys['c2statesdd']))
 	{
		unset($_SESSION[SUMAC_USERPAR_STATESDD]['courses']);
		$temp = sumac_get_parameter(null,'c2statesdd',(isset($getKeys['c2statesdd']) ? $getKeys['c2statesdd'] : null));
		if ($temp != null) $_SESSION[SUMAC_USERPAR_STATESDD]['courses'] = $temp;
	}

 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['ticketing']);
		$temp = sumac_get_parameter_from_file(null,'t2nouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['ticketing'] = (strtolower(substr($temp,0,1)) !== 'f');
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['ticketing']);
		$temp = sumac_get_parameter_from_file(null,'t2omitfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['ticketing'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['ticketing']);
		$temp = sumac_get_parameter_from_file(null,'t2mustfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['ticketing'] = strtolower($temp);
	}
 	if (($resume === false) || isset($getKeys['t2statesdd']))
 	{
		unset($_SESSION[SUMAC_USERPAR_STATESDD]['ticketing']);
		$temp = sumac_get_parameter(null,'t2statesdd',(isset($getKeys['t2statesdd']) ? $getKeys['t2statesdd'] : null));
		if ($temp != null) $_SESSION[SUMAC_USERPAR_STATESDD]['ticketing'] = $temp;
	}
	//and repeat settings for 'ticketing2' package, i.e. same parameters used for both packages
	if (isset($_SESSION[SUMAC_USERPAR_NOUSERPW]['ticketing'])) $_SESSION[SUMAC_USERPAR_NOUSERPW]['ticketing2'] = $_SESSION[SUMAC_USERPAR_NOUSERPW]['ticketing'];
	else unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['ticketing2']);
	if (isset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['ticketing'])) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['ticketing2'] = $_SESSION[SUMAC_USERPAR_OMITFIELDS]['ticketing'];
	else unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['ticketing2']);
	if (isset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['ticketing'])) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['ticketing2'] = $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['ticketing'];
	else unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['ticketing2']);
	if (isset($_SESSION[SUMAC_USERPAR_STATESDD]['ticketing'])) $_SESSION[SUMAC_USERPAR_STATESDD]['ticketing2'] = $_SESSION[SUMAC_USERPAR_STATESDD]['ticketing'];
	else unset($_SESSION[SUMAC_USERPAR_STATESDD]['ticketing2']);

// set parameters for new grantreview package alone
//set these new grantreview _SESSION globals only if there is a parameter setting
	if ($resume === false)	//MUST COME FROM FILE
	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['grantreview2g']);
		$temp = sumac_get_parameter_from_file(null,'r2gnouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['grantreview2g'] = (strtolower(substr($temp,0,1)) !== 'f');
	}
	if ($resume === false)	//MUST COME FROM FILE
	{
		unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['grantreview2g']);
		$temp = sumac_get_parameter_from_file(null,'r2gomitfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['grantreview2g'] = strtolower($temp);
	}
	if ($resume === false)	//MUST COME FROM FILE
	{
		unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['grantreview2g']);
		$temp = sumac_get_parameter_from_file(null,'r2gmustfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['grantreview2g'] = strtolower($temp);
	}
	if (($resume === false) || isset($getKeys['r2gstatesdd']))
	{
		unset($_SESSION[SUMAC_USERPAR_STATESDD]['grantreview2g']);
		$temp = sumac_get_parameter(null,'r2gstatesdd',(isset($getKeys['r2gstatesdd']) ? $getKeys['r2gstatesdd'] : null));
		if ($temp != null) $_SESSION[SUMAC_USERPAR_STATESDD]['grantreview2g'] = $temp;
	}
	if ($resume === false)	//MUST COME FROM FILE
	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['grantreview2r']);
		$temp = sumac_get_parameter_from_file(null,'r2rnouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['grantreview2r'] = (strtolower(substr($temp,0,1)) !== 'f');
	}

// set parameters for new signup package alone
//set these new signup _SESSION globals only if there is a parameter setting
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_NOUSERPW]['signup2']);
		$temp = sumac_get_parameter_from_file(null,'s2nouserpw');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_NOUSERPW]['signup2'] = (strtolower(substr($temp,0,1)) !== 'f');
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_OMITFIELDS]['signup2']);
		$temp = sumac_get_parameter_from_file(null,'s2omitfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_OMITFIELDS]['signup2'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_MUSTFIELDS]['signup2']);
		$temp = sumac_get_parameter_from_file(null,'s2mustfields');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_MUSTFIELDS]['signup2'] = strtolower($temp);
	}
 	if (($resume === false) || isset($getKeys['s2statesdd']))
 	{
		unset($_SESSION[SUMAC_USERPAR_STATESDD]['signup2']);
		$temp = sumac_get_parameter(null,'s2statesdd',(isset($getKeys['s2statesdd']) ? $getKeys['s2statesdd'] : null));
		if ($temp != null) $_SESSION[SUMAC_USERPAR_STATESDD]['signup2'] = $temp;
	}

// set parameters that supply an exit URL for each package
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['anypackage']);
		$temp = sumac_get_parameter_from_file(null,'exiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['anypackage'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['donation']);
		$temp = sumac_get_parameter_from_file(null,'donexiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['donation'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['ticketing']);
		$temp = sumac_get_parameter_from_file(null,'tktexiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['ticketing'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['courses']);
		$temp = sumac_get_parameter_from_file(null,'crsexiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['courses'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['directories']);
		$temp = sumac_get_parameter_from_file(null,'direxiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['directories'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['membership']);
		$temp = sumac_get_parameter_from_file(null,'memexiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['membership'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['contact']);
		$temp = sumac_get_parameter_from_file(null,'conexiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['contact'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['donation2']);
		$temp = sumac_get_parameter_from_file(null,'d2exiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['donation2'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['singleform2']);
		$temp = sumac_get_parameter_from_file(null,'f2exiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['singleform2'] = strtolower($temp);
	}
	if ($resume === false)	//MUST COME FROM FILE
	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['grantreview2g']);
		$temp = sumac_get_parameter_from_file(null,'r2gexiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['grantreview2g'] = strtolower($temp);
	}
	if ($resume === false)	//MUST COME FROM FILE
	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['grantreview2r']);
		$temp = sumac_get_parameter_from_file(null,'r2rexiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['grantreview2r'] = strtolower($temp);
	}
 	if ($resume === false)	//MUST COME FROM FILE
 	{
		unset($_SESSION[SUMAC_USERPAR_EXITURL]['signup2']);
		$temp = sumac_get_parameter_from_file(null,'s2exiturl');
		if ($temp != null) $_SESSION[SUMAC_USERPAR_EXITURL]['signup2'] = strtolower($temp);
	}

// set parameter that defines action to be taken after payment in any new package (e.g. donation2, membership2)
 	if (($resume === false) || isset($getKeys['exitlogout']))
		$_SESSION[SUMAC_USERPAR_EXITLOGOUT] = (substr(strtolower(sumac_get_parameter('false','exitlogout',(isset($getKeys['exitlogout']) ? $getKeys['exitlogout'] : null))),0,1) != 'f');
...
// set parameters for connection to Sumac
 	...
 	if (($resume === false) || isset($getKeys['nograntform']))
		$_SESSION[SUMAC_SESSION_EXCLUDE_GRANTFORMS] = (substr(strtolower(sumac_get_parameter('false','nograntform',(isset($getKeys['nograntform']) ? $getKeys['nograntform'] : null))),0,1) != 'f');
...
function sumac_use_single_package_parameter_defaults()
{
	...
	$_SESSION[SUMAC_SESSION_EXCLUDE_THEATRE] = ($_SESSION[SUMAC_SESSION_ACTIVE_PACKAGE] != SUMAC_PACKAGE_TICKETING)
												&& ($_SESSION[SUMAC_SESSION_ACTIVE_PACKAGE] != SUMAC_PACKAGE_TICKETING2);
	...
	$_SESSION[SUMAC_SESSION_EXCLUDE_GRANTFORMS] = (($_SESSION[SUMAC_SESSION_ACTIVE_PACKAGE] != SUMAC_PACKAGE_GRANTREVIEW2G)
