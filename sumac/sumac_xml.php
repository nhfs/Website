...
//removeCommentsAndDTDs($organisationDocument);
	...
	$_SESSION[SUMAC_SESSION_SUPPORTED_FEATURES] = sumac_getElementValuesAsArray($organisationDocument,SUMAC_ELEMENT_FEATURE);
	return $organisationDocument;
}

function sumac_loadLoginAccountDocument($source,$port,$xml,$request,$includes)
{
	...
	if ($includes != null)
	{
		if (is_array($includes))
		{
			...
			{
				 $variableValues .= '&' . SUMAC_REQUEST_KEYWORD_INCLUDE . '=' . $includes[$i];
			}
		}
		else
		{
			 $variableValues .= '&' . SUMAC_REQUEST_KEYWORD_INCLUDE . '=' . $includes;
		}
	}
...
function sumac_unloadExtrasDocument()
{
	unset($_SESSION[SUMAC_SESSION_EXTRAS]);
}

function sumac_loadFormTemplateDocument($source,$port,$request,$form,$formowner)
{
	$variableValues1 = SUMAC_REQUEST_KEYWORD_REQUEST . '=' . $request . $_SESSION[SUMAC_SESSION_WEBSITE_DATA];
	$variableValues2 = SUMAC_REQUEST_KEYWORD_CONTACTID . '=' . $_SESSION[SUMAC_SESSION_CONTACT_ID];
	$variableValues3 = SUMAC_REQUEST_KEYWORD_FORM . '=' . $form;
	$variableValues4 = SUMAC_REQUEST_KEYWORD_FORMOWNER . '=' . $formowner;
	$formTemplateData = sumac_getXMLData($source,$port,'?' . $variableValues1
						. '&' . $variableValues2 . '&' . $variableValues3 . '&' . $variableValues4);
...
function sumac_getFormtemplateElement($formId,$tryInOrgDoc)
{
	$formTemplateElement = false;
	if (isset($_SESSION[SUMAC_SESSION_FORM][$formId]))
	{
	...
	else if ($tryInOrgDoc) //try for a copy in the organisation doc
	{
		$organisationDocument = sumac_reloadOrganisationDocument();
		if ($organisationDocument != false)
		{
			$formTemplateElements = $organisationDocument->getElementsByTagName(SUMAC_ELEMENT_FORMTEMPLATE);
			for ($i = 0; $i < $formTemplateElements->length; $i++)
			{
				if ($formId == $formTemplateElements->item($i)->getAttribute(SUMAC_ATTRIBUTE_ID))
				{
					$formTemplateElement = $formTemplateElements->item($i);
					break;
				}
			}
		}
...
function sumac_loadAttachmentDocument($source,$port,$attachmentparam,$keyword,$xml,$decode=true)
{
	$variableValues1 = SUMAC_REQUEST_KEYWORD_REQUEST . '=' . $attachmentparam . $_SESSION[SUMAC_SESSION_WEBSITE_DATA];
	$variableValues2 = $keyword . '=' . urlencode($xml);
	$attachmentData = sumac_getXMLData($source,$port,'?' . $variableValues1 . '&' . $variableValues2);
	if ($attachmentData == false) return false;

	sumac_echoRawXMLIfDebugging($attachmentData,$attachmentparam);

	$attachmentDocument = sumac_loadXMLDoc($attachmentData);
	if ($attachmentDocument == false)
	{
		$_SESSION[SUMAC_SESSION_REQUEST_ERROR] .= SUMAC_ERROR_ATTACHMENT_DATA_NOT_XML . $_SESSION[SUMAC_STR]["AE5"];
		return false;
	}
	if (!$attachmentDocument->validate())
	{
		$_SESSION[SUMAC_SESSION_FATAL_ERROR] = $_SESSION[SUMAC_SESSION_INVALID_SERVER_RESPONSE];
		$_SESSION[SUMAC_SESSION_REQUEST_ERROR] = SUMAC_ERROR_ATTACHMMENT_NOT_VALID . $_SESSION[SUMAC_STR]["AE5"];
		return false;
	}

	if ($decode) sumac_decodeDocumentContent($attachmentDocument);
//*************************************************
//decoding the document has a disastrous effect on the Base64-encoded data in it - pluses become spaces
//possibly the other fields that weren't Base64-encoded SHOULD be decoded - but at present they aren't being

	if (sumac_handleResponseXMLDocument($attachmentDocument,SUMAC_ELEMENT_ATTACHMENT) == false) return false;
	else if (sumac_isExpectedXMLDocument($attachmentDocument,SUMAC_ELEMENT_ATTACHMENT) == false) return false;

//	$_SESSION[SUMAC_SESSION_ATTMT_ID] = $attachmentDocument->documentElement->getAttribute(SUMAC_ATTRIBUTE_ID);
	return $attachmentDocument;
}

function sumac_postRequestAndloadAttachmentDocument($source,$port,$request,$xml)
{
	$variableValues = SUMAC_REQUEST_KEYWORD_REQUEST . '=' . $request . $_SESSION[SUMAC_SESSION_WEBSITE_DATA];
//	$attachmentData = sumac_postEncryptedXMLData($source,$port, '?' . $variableValues,$xml,'sumac.pem');
	$attachmentData = sumac_postUnencryptedXMLData($source,$port, '?' . $variableValues,$xml);
	if ($attachmentData == false) return false;

	sumac_echoRawXMLIfDebugging($attachmentData,$request);
	...
	sumac_decodeDocumentContent($attachmentDocument);
	if (sumac_handleResponseXMLDocument($attachmentDocument,SUMAC_ELEMENT_ATTACHMENT) == false) return false;
	else if (sumac_isExpectedXMLDocument($attachmentDocument,SUMAC_ELEMENT_ATTACHMENT) == false) return false;
...
//	$responseData = sumac_postUnencryptedXMLData($source,$port, '?' . $variableValues,$xml);
	...
	if ($document->documentElement->tagName == SUMAC_ELEMENT_RESPONSE) //embedded error message
	{
		...
		if ($responseStatus == 'bad')
		{
			...
			if ($responseMessageElements->length == 0) $responseMessage = $defaultMessage;
									...
									: $defaultMessage;
			$responseMessagecodeElements = $document->getElementsByTagName(SUMAC_ELEMENT_MESSAGECODE);
...
//echo $xmldata . '<br />';
	...
	//$result = $doc->loadXML($xmldata,LIBXML_DTDLOAD | LIBXML_DTDVALID);
