...
function sumac_payment2($pkg,$buttoncode,$fixedPaymentAmount,$monthly,$organisationDocument,$accountDocument,$previousRequest,$responseMessage)
{
	return sumac_payment2_section($pkg,$buttoncode,$fixedPaymentAmount,$organisationDocument)
		.sumac_payment2_pay_section($pkg,$buttoncode,$fixedPaymentAmount,$monthly,$organisationDocument,$accountDocument,$previousRequest,$responseMessage);
}

function sumac_payment2_section($pkg,$buttoncode,$fixedPaymentAmount,$organisationDocument)
{
	$html = sumac_geth2_divtag($pkg,'payment','titled');
	if ($fixedPaymentAmount == '0')
	{
		$html .= sumac_geth2_sumac_title_with_line($pkg,'PH2');
		$attrs = 'type="hidden" value=""';
		$html .= sumac_geth2_input('sumac_input_'.$pkg.'cardtype','hidden','cardtype',$attrs);
		$html .= sumac_geth2_input('sumac_input_'.$pkg.'cardnumber','hidden','cardnumber',$attrs);
		$html .= sumac_geth2_input('sumac_input_'.$pkg.'cardsecurity','hidden','cardsecurity',$attrs);
		$html .= sumac_geth2_input('sumac_input_'.$pkg.'cardexpmonth','hidden','cardexpmonth',$attrs);
		$html .= sumac_geth2_input('sumac_input_'.$pkg.'cardexpyear','hidden','cardexpyear',$attrs);
		$html .= sumac_geth2_input('sumac_input_'.$pkg.'carduser','hidden','carduser',$attrs);
	}
	else
	{
...
function sumac_payment2_pay_section($pkg,$buttoncode,$fixedPaymentAmount,$monthly,$organisationDocument,$accountDocument,$previousRequest,$responseMessage)
{
	$html = sumac_geth2_divtag($pkg,'pay','untitled');

	$labelId = 'PL1';
	if ($fixedPaymentAmount == '') $labelId = 'PL2';
	else if ($fixedPaymentAmount == '0') $labelId = 'PL4';
	$html .= sumac_payment2_warning($pkg,$labelId,$fixedPaymentAmount,$monthly);
	...
	$html .= sumac_geth2_tdtag($pkg,'paybutton',array('pay_button','titlebutton'));
	$onclick = 'sumac_'.$pkg.'_set_amountpaid();'
				.' if (sumac_check_for_missing_fields(\''.$buttoncode.'\',\''.sumac_geth2_textid_for_span($pkg,(($accountDocument == null)?'PE1':'PE2')).'\'))'
					.' return false;'
					.' else sumac_cover_mainpage(\''.$pkg.'\');';
	$attrs = 'type="submit" value="" onclick="'.$onclick.'"';
	$html .= sumac_geth2_input(sumac_geth2_textid_for_entry($pkg,'input',$labelId),'submit','pay',$attrs);
	$html .= '</td>'.PHP_EOL;
...
function sumac_payment2_warning($pkg,$labelId,$amount,$monthly)
{
	$html = sumac_geth2_divtag($pkg,'warning','warning');
	$messageId = 'PI1';  /*($monthly?'PI2':'PI1')*/
	if ($amount == '0') $messageId = 'PI3';
	$html .= sumac_geth2_span($pkg,null,'warning',$messageId,true,array('SUMACID'.sumac_geth2_fulltextid($pkg,$labelId),sumac_centsToPrintableDollars($amount)));
	$html .= '</div>'.PHP_EOL;
	return $html;
}
