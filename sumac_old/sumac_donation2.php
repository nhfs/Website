...
function sumac_donation2_content($organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage)
{
	...
	$html .= sumac_geth2_sumac_software_link('donation2','/fundraising-software',SUMAC_INFO_FOOTER_SUMAC_DONATION2_TEXT);
...
function sumac_donation2_HTMLform($organisationDocument,$formhandler,$accountDocument,$previousRequest,$responseMessage)
{
	...
	$html .= sumac_login2('donation2','D',$_SESSION[SUMAC_USERPAR_D2NOLOGIN],$accountDocument,$previousRequest,$responseMessage,null);
...
function sumac_donation2_amount_section($organisationDocument)
{
	...
	$monthlyoptional = ($_SESSION[SUMAC_USERPAR_D2FREQUENCY] == 'C');
	$deductiondays = sumac_getElementValuesAsArray($organisationDocument,SUMAC_ELEMENT_DEDUCTION_DAY);
	if ($monthlyoptional || (($_SESSION[SUMAC_USERPAR_D2FREQUENCY] == 'M') && (count($deductiondays) >= 1))) $html .= sumac_donation2_monthly_table($deductiondays);
	$html .= '</div>'.PHP_EOL;
	return $html;
}
...
function sumac_donation2_monthly_table($deductiondays)
{
	...
	if ($_SESSION[SUMAC_USERPAR_D2FREQUENCY] == 'C')
	{
	...
	$attrs = 'type="hidden" value="'.(($_SESSION[SUMAC_USERPAR_D2FREQUENCY] == 'M')?'1':'-1').'"';
	$html .= sumac_geth2_input('sumac_input_donation2_frequency','hidden','frequency',$attrs);
	$html .= '</td>'.PHP_EOL;

	if ((count($deductiondays) >= 1)
		&& (($_SESSION[SUMAC_USERPAR_D2FREQUENCY] == 'C') || ($_SESSION[SUMAC_USERPAR_D2FREQUENCY] == 'M')))
	{
